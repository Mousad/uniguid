...
function StatCard({ stat, index, inView }: { stat: typeof stats[0]; index: number; inView: boolean }) {
  ...
  return (
    ...
    >
      <div
        className="w-10 h-10 rounded-[20px] flex items-center justify-center mb-5 transition-all duration-300 group-hover:scale-110"
        style={{ background: `linear-gradient(135deg, ${stat.color}15, ${stat.color}30)` }}
      >
        <Icon size={23} style={{ color: stat.color }} strokeWidth={1} />
      </div>
     
      <h3 className="text-gray-800  font-bold text-[15px] mb-2">{stat.label}</h3>
      <p className="text-gray-500 text-[10px] leading-relaxed">{stat.description}</p>
       <div
...
export function StatsSection() {
  ...
  return (
  <section ref={ref} className="py-10 px-4" style={{ backgroundColor: "#fefcfd" }}>
